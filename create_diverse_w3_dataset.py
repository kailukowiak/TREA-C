...
def create_diverse_sampling_strategy(df_lazy, target_rows=5_000_000):
    ...
    for i in range(num_chunks):
        ...
        random_offset = np.random.randint(0, min(step_size // 4, 100_000)) if step_size > 100_000 else 0
...
def analyze_dataset_quality(df):
    ...
    well_state_counts = df.group_by("well_name").agg([
        pol.col("state").n_unique().alias("unique_states"),
        pol.len().alias("row_count")
    ]).sort("row_count", descending=True)
    
    print(f"\nWell statistics:")
    print(f"  Wells with multiple states: {(well_state_counts['unique_states'] > 1).sum()}")
...
def save_improved_dataset(df, output_path="/home/ubuntu/DuET/data/W3/improved_train_5M.parquet"):
