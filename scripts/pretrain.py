"""Pre-training script for variable feature time series model using self-supervised learning.
...
from duet.data.multi_dataset_loader import MultiDatasetDataModule
...
def create_datasets(target_sequence_length: int = 96) -> Dict[str, Dict[str, any]]:
    ...
    train_datasets = {
        ...
        "human_activity": HumanActivityDataset(split="train", seq_len=target_sequence_length),
        "air_quality": AirQualityDataset(split="train", seq_len=target_sequence_length),
        "financial_market": FinancialMarketDataset(split="train", seq_len=target_sequence_length),
    }
    
    # Create validation datasets
    val_datasets = {
        "etth1": ETTh1Dataset(train=False, sequence_length=target_sequence_length),
        "human_activity": HumanActivityDataset(split="val", seq_len=target_sequence_length),
        "air_quality": AirQualityDataset(split="val", seq_len=target_sequence_length),
        "financial_market": FinancialMarketDataset(split="val", seq_len=target_sequence_length),
    }
    ...
    for name, dataset in train_datasets.items():
        ...
        if hasattr(dataset, 'numeric_features'):
            ...
            categorical_features = sample['x_cat'].shape[0] if 'x_cat' in sample and sample['x_cat'] is not None else 0
            num_classes = getattr(dataset, 'num_classes', 3)
            sequence_length = getattr(dataset, 'sequence_length', sample['x_num'].shape[1] if 'x_num' in sample else 96)
        
        print(f"  Features: {numeric_features} numeric, {categorical_features} categorical")
...
def create_model(datasets: Dict[str, Dict[str, any]], args) -> PretrainPatchDuET:
    ...
    if not any([args.use_masked_patch, args.use_temporal_order, args.use_contrastive,
                args.no_masked_patch, args.no_temporal_order, args.no_contrastive]):
...
def main():
    ...
    # Data parameters
    parser.add_argument("--sequence_length", type=int, default=96, help="Target sequence length")
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size")
    parser.add_argument("--num_workers", type=int, default=4, help="Number of data workers")
    parser.add_argument("--oversample_smaller", action="store_true", default=True, help="Oversample smaller datasets")
    ...
    parser.add_argument("--use_temporal_order", action="store_true", help="Use temporal order prediction") 
    parser.add_argument("--use_contrastive", action="store_true", help="Use contrastive learning")
    parser.add_argument("--no_masked_patch", action="store_true", help="Disable masked patch prediction")
    parser.add_argument("--no_temporal_order", action="store_true", help="Disable temporal order prediction")
    parser.add_argument("--no_contrastive", action="store_true", help="Disable contrastive learning")
    
    parser.add_argument("--mask_ratio", type=float, default=0.15, help="Masking ratio for MPP")
    parser.add_argument("--temporal_shuffle_ratio", type=float, default=0.3, help="Shuffle ratio for temporal order")
    parser.add_argument("--contrastive_temperature", type=float, default=0.1, help="Contrastive temperature")
    parser.add_argument("--augmentation_strength", type=float, default=0.1, help="Data augmentation strength")
    
    # Loss weights
    parser.add_argument("--lambda_masked", type=float, default=1.0, help="Weight for masked patch loss")
    parser.add_argument("--lambda_temporal", type=float, default=0.5, help="Weight for temporal order loss")
    parser.add_argument("--lambda_contrastive", type=float, default=0.3, help="Weight for contrastive loss")
    parser.add_argument("--lambda_supervised", type=float, default=0.1, help="Weight for supervised loss")
    
    # Training parameters
    parser.add_argument("--max_epochs", type=int, default=50, help="Maximum training epochs")
    parser.add_argument("--patience", type=int, default=10, help="Early stopping patience")
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints/pretrain", help="Checkpoint directory")
    parser.add_argument("--log_dir", type=str, default="./logs/pretrain", help="Logging directory")
    ...
    print(f"SSL objectives: {[obj for obj, enabled in model.ssl_objectives_config.items() if enabled]}")
    ...
    print(f"model = PretrainPatchDuET.from_pretrained('{final_checkpoint_path}', num_classes=<your_classes>)")
