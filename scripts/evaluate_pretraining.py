...
def get_dataset(dataset_name: str, split: str, sequence_length: int):
    ...
    datasets = {
        "etth1": lambda: ETTh1Dataset(train=(split == "train"), sequence_length=sequence_length),
        "human_activity": lambda: HumanActivityDataset(split=split, seq_len=sequence_length),
        "air_quality": lambda: AirQualityDataset(split=split, seq_len=sequence_length),
        "financial_market": lambda: FinancialMarketDataset(split=split, seq_len=sequence_length)
    }
...
def load_model_from_checkpoint(checkpoint_path: str, dataset, model_type: str = "pretrained"):
    ...
    if model_type == "pretrained":
        ...
        model = VariableFeaturePatchDuET(
            ...
            num_classes=dataset.num_classes,
            **{k: v for k, v in hparams.items() if k not in ['max_numeric_features', 'max_categorical_features', 'num_classes']}
        )
    ...
    if hasattr(model, 'set_dataset_schema'):
        model.set_dataset_schema(
            ...
            column_names=dataset.get_column_names() if hasattr(dataset, 'get_column_names') else None
        )
...
def run_evaluation_suite(args):
    ...
    for dataset_name in datasets:
        ...
        pretrained_path = Path(args.checkpoint_dir) / f"finetune_{dataset_name}" / "pretrained_freeze_False" / "best*.ckpt"
        pretrained_checkpoints = list(pretrained_path.parent.glob("best*.ckpt")) if pretrained_path.parent.exists() else []
        
        if pretrained_checkpoints:
            print("Evaluating pre-trained model...")
            try:
                model = load_model_from_checkpoint(str(pretrained_checkpoints[0]), test_dataset, "pretrained")
                pretrained_results = evaluate_model(model, test_loader, args.device)
                dataset_results['pretrained'] = pretrained_results
                print(f"  Pre-trained - Loss: {pretrained_results['loss']:.4f}, Accuracy: {pretrained_results['accuracy']:.4f}")
        ...
        scratch_path = Path(args.checkpoint_dir) / f"finetune_{dataset_name}" / "from_scratch" / "best*.ckpt"
        scratch_checkpoints = list(scratch_path.parent.glob("best*.ckpt")) if scratch_path.parent.exists() else []
        
        if scratch_checkpoints:
            print("Evaluating from-scratch model...")
            try:
                model = load_model_from_checkpoint(str(scratch_checkpoints[0]), test_dataset, "scratch")
                scratch_results = evaluate_model(model, test_loader, args.device)
                dataset_results['from_scratch'] = scratch_results
                print(f"  From-scratch - Loss: {scratch_results['loss']:.4f}, Accuracy: {scratch_results['accuracy']:.4f}")
            except Exception as e:
...
def generate_report(results: List[Dict], output_dir: str):
    ...
    for result in results:
        ...
        if result['pretrained'] and result['from_scratch']:
            loss_improvement = (result['from_scratch']['loss'] - result['pretrained']['loss']) / result['from_scratch']['loss'] * 100
            acc_improvement = (result['pretrained']['accuracy'] - result['from_scratch']['accuracy']) / result['from_scratch']['accuracy'] * 100
    ...
    with open(report_path, 'w') as f:
        ...
        for result in results:
            ...
            if result['pretrained'] and result['from_scratch']:
                loss_imp = (result['from_scratch']['loss'] - result['pretrained']['loss']) / result['from_scratch']['loss'] * 100
                acc_imp = (result['pretrained']['accuracy'] - result['from_scratch']['accuracy']) / result['from_scratch']['accuracy'] * 100
        ...
        valid_improvements = [row for row in summary_data if row['Loss_Improvement_%'] is not None]
        if valid_improvements:
            avg_loss_imp = sum(row['Loss_Improvement_%'] for row in valid_improvements) / len(valid_improvements)
            avg_acc_imp = sum(row['Accuracy_Improvement_%'] for row in valid_improvements) / len(valid_improvements)
            
            f.write(f"Average Loss Improvement: {avg_loss_imp:.2f}%\n")
            f.write(f"Average Accuracy Improvement: {avg_acc_imp:.2f}%\n")
            
            num_better_loss = sum(1 for row in valid_improvements if row['Loss_Improvement_%'] > 0)
            num_better_acc = sum(1 for row in valid_improvements if row['Accuracy_Improvement_%'] > 0)
            
            f.write(f"Datasets with better loss: {num_better_loss}/{len(valid_improvements)}\n")
            f.write(f"Datasets with better accuracy: {num_better_acc}/{len(valid_improvements)}\n")
