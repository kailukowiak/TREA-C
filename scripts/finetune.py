...
def create_pretrained_model(pretrained_path: str, num_classes: int, freeze_encoder: bool = True) -> PretrainPatchDuET:
...
def create_dataloaders(dataset_name: str, sequence_length: int, batch_size: int, num_workers: int):
    ...
    try:
        ...
        try:
            ...
            val_dataset = get_dataset(dataset_name, "train", sequence_length)  # This will create val split
            val_dataset.split = "val"  # Override split
    
    print(f"Dataset info:")
    print(f"  Train samples: {len(train_dataset)}")
    print(f"  Val samples: {len(val_dataset)}")
    print(f"  Features: {train_dataset.numeric_features} numeric, {train_dataset.categorical_features} categorical")
...
def run_experiment(model, train_loader, val_loader, dataset, args, experiment_name: str):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fine-tune pre-trained time series model")
    ...
    parser.add_argument("--compare_from_scratch", action="store_true", default=True,
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints/finetune", help="Checkpoint directory")
    parser.add_argument("--log_dir", type=str, default="./logs/finetune", help="Logging directory")
    ...
    if args.pretrained_path and Path(args.pretrained_path).exists():
        ...
        result = run_experiment(model, train_loader, val_loader, dataset, args, experiment_name)
        ...
        if args.freeze_encoder:
            ...
            result = run_experiment(model_unfrozen, train_loader, val_loader, dataset, args, experiment_name)
            results.append(result)
    ...
    if args.compare_from_scratch:
        ...
        result = run_experiment(model_scratch, train_loader, val_loader, dataset, args, experiment_name)
        results.append(result)
    ...
    for result in results:
        ...
        print(f"  Best validation loss: {result['best_val_loss']:.4f}" if result['best_val_loss'] else "  N/A")
    ...
    # Find best result
    if results:
        best_result = min(results, key=lambda x: x['best_val_loss'] if x['best_val_loss'] else float('inf'))
