...
def find_diverse_state_regions():
    ...
    for pos in search_positions:
        try:
            chunk = df_lazy.slice(pos, chunk_size).select(["state", "well_name"]).collect()
            ...
            if len(chunk) > 0:
                ...
                if unique_states > 1:
                    ...
                    print(f"✅ Position {pos:,}: {unique_states} states {state_list}, {wells} wells")
    ...
    if "P-PDG" in df_sample.columns:
        ...
        if len(pressure_values) > 1000:
            ...
            df_with_target = df_sample.with_columns([
                pol.when(pol.col("P-PDG") < q25).then(0)
                  .when(pol.col("P-PDG") > q75).then(2)
                  .otherwise(1)
                  .alias("target")
...
def save_and_test_dataset(df, strategy_name):
    ...
    # Save main dataset
    output_path = f"/home/ubuntu/DuET/data/W3/multiclass_{strategy_name.lower()}.parquet"
