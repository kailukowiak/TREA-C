...
def real_world_experiment():
    ...
    datasets_config = {
        ...
        "NASA_Turbofan": {
            ...
            "train_args": {"train": True, "sequence_length": 80, "task": "classification"},
            "val_args": {"train": False, "sequence_length": 80, "task": "classification"},
    ...
    for name, config in datasets_config.items():
        print(f"  {name}: {config['expected_features']} features, {config['expected_classes']} classes - {config['description']}")
    ...
    for dataset_name, config in datasets_config.items():
        ...
        try:
            ...
            actual_schemas[dataset_name] = {
                ...
                "classes": train_dataset.num_classes if hasattr(train_dataset, 'num_classes') else config['expected_classes'],
                "seq_len": x_shape[1]
            }
            
            print(f"  ✅ Schema: {actual_schemas[dataset_name]['numeric']} numeric features")
    ...
    for dataset_name, config in datasets_config.items():
        ...
        try:
            ...
            print(f"  Features: {schema['numeric']} numeric + {schema['categorical']} categorical")
    ...
    display_columns = [
        "Dataset", "Description", "Total Features", "Classes", 
        "Sequence Length", "Accuracy", "F1 Score", "Training Time (s)"
    ]
    ...
    print(f"Sequence length range: {df['Sequence Length'].min()}-{df['Sequence Length'].max()} timesteps")
    ...
    print(f"   ✅ Feature range: {df['Total Features'].min()}-{df['Total Features'].max()} features")
    print(f"   ✅ Average performance: {avg_accuracy:.1%}")
    print(f"   ✅ Variable sequence lengths: {df['Sequence Length'].min()}-{df['Sequence Length'].max()} timesteps")
