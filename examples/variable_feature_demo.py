...
def create_variable_synthetic_dataset(
    ...
    if categorical_features > 0:
        x_cat = torch.randint(
            0, 5, (num_samples, categorical_features, seq_len)
        ).float()
...
def variable_feature_experiment():
    ...
    for name, config in dataset_configs.items():
        print(
            f"  {name}: {config['numeric']} numeric + {config['categorical']} "
            f"categorical = {config['numeric'] + config['categorical']} total"
        )
    ...
    for dataset_name, config in dataset_configs.items():
        ...
        print(
            f"  Schema: {config['numeric']} numeric + "
            f"{config['categorical']} categorical"
        )
        print(f"  Columns: {config['columns']}")
        print(f"  Train samples: {len(train_dataset)}, Val samples: {len(val_dataset)}")

        # Train model
        result = train_variable_model(
            model, dm, dataset_name.replace(" ", "_"), max_epochs=5
        )
    ...
    print("\n🎯 CONCLUSION:")
    print(
        f"   ✅ Single model handles {df['Total Features'].min()}-"
        f"{df['Total Features'].max()} features seamlessly"
    )
    print(f"   ✅ Consistent performance: {avg_accuracy:.1%} average accuracy")
    print(
        f"   ✅ Variable schema support: {df['Numeric Features'].min()}-"
        f"{df['Numeric Features'].max()} numeric, {df['Categorical Features'].min()}-"
        f"{df['Categorical Features'].max()} categorical"
    )
    print("   🚀 READY FOR MULTI-DATASET DEPLOYMENT!")
