...
def train_model(model, dm, model_name, max_epochs=30):
    ...
    logger = TensorBoardLogger(
        save_dir='logs',
        name=f'turbofan_comparison/{model_name}'
    )
...
def main():
    ...
    try:
        ...
        train_dataset = SyntheticTimeSeriesDataset(
            num_samples=5000, T=50, C_num=21, C_cat=2,
            cat_cardinalities=[5, 5], num_classes=3,
            task="classification", missing_ratio=0.05
        )
        val_dataset = SyntheticTimeSeriesDataset(
            num_samples=1000, T=50, C_num=21, C_cat=2,
            cat_cardinalities=[5, 5], num_classes=3,
            task="classification", missing_ratio=0.05
        )
    ...
    print(f"Best Accuracy: {best_acc_model} ({df.loc[df['Model']==best_acc_model, 'Accuracy'].values[0]:.3f})")
    print(f"Best F1 Score: {best_f1_model} ({df.loc[df['Model']==best_f1_model, 'F1 Score'].values[0]:.3f})")
    print(f"Fastest Training: {fastest_model} ({df.loc[df['Model']==fastest_model, 'Training Time (s)'].values[0]:.1f}s)")
    print(f"Smallest Model: {smallest_model} ({df.loc[df['Model']==smallest_model, 'Parameters'].values[0]:,} params)")
