...
class PretrainPatchDuET(VariableFeaturePatchDuET):
    """VariableFeaturePatchDuET extended with self-supervised learning for pre-training."""
    ...
    def __init__(
        ...
        ssl_objectives: Optional[Dict[str, bool]] = None,
        ...
        lambda_supervised: float = 0.1,  # Weight for supervised loss during pre-training
        ...
        **kwargs
        ...
        if ssl_objectives.get('temporal_order', False):
            ...
            self.ssl_heads['temporal_order'] = nn.Sequential(
                nn.Linear(self.hparams.d_model, self.hparams.d_model),
                nn.ReLU(),
                nn.Linear(self.hparams.d_model, self.hparams.d_model)  # Will project to num_patches in forward
            )
    ...
    def forward_ssl(
        ...
        if 'masked_patch' in self.ssl_objectives_config and self.ssl_objectives_config['masked_patch']:
            # Create mask
        ...
        if 'temporal_order' in self.ssl_objectives_config and self.ssl_objectives_config['temporal_order']:
        ...
        if 'contrastive' in self.ssl_objectives_config and self.ssl_objectives_config['contrastive']:
            # Original embedding
