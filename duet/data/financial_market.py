        ...
        for market_id, market_type in enumerate(self.MARKET_LABELS):
            for _ in range(samples_per_class):
                ...
                price_changes = np.random.normal(
                    trend * dt, volatility_base * np.sqrt(dt), self.seq_len
                )
                ...
                # OHLC data
                open_prices = prices
                close_prices = np.roll(
                    prices, -1
                )  # Next period's open becomes current close
                close_prices[-1] = prices[-1] * (
                    1 + np.random.normal(trend * dt, volatility_base * np.sqrt(dt))
                )

                # High and low with realistic spreads
                high_prices = np.maximum(
                    open_prices, close_prices
                ) + np.random.exponential(0.5, self.seq_len)
                ...
                if market_type == "bear":
                    volume = base_volume * (
                        2 - (prices / prices[0])
                    ) + 200000 * np.random.rand(self.seq_len)
                else:
                ...
                volatility = np.convolve(
                    volatility, np.ones(5) / 5, mode="same"
                )  # 5-period smoothing
                ...
                rsi = (
                    50
                    + 30 * np.sin(np.linspace(0, 4 * np.pi, self.seq_len))
                    + 10 * np.random.randn(self.seq_len)
                )
                ...
                # Bollinger Bands
                moving_avg_20 = np.convolve(
                    close_prices,
                    np.ones(min(20, self.seq_len)) / min(20, self.seq_len),
                    mode="same",
                )
                ...
                # Moving averages
                moving_avg_50 = np.convolve(
                    close_prices,
                    np.ones(min(50, self.seq_len)) / min(50, self.seq_len),
                    mode="same",
                )
...
if __name__ == "__main__":
    ...
    dataset = FinancialMarketDataset(split="train", seq_len=60, download=True)
