...
class MultiDatasetSampler(Sampler):
    ...
    def __init__(
        ...
        self.dataset_weights = {
            name: size / total_samples for name, size in dataset_sizes.items()
        }
...
class MultiDatasetCollection(Dataset):
    ...
    def __init__(
        ...
        if max_numeric_features is not None:
            ...
            self.max_numeric_features = max(
                info["numeric_features"] for info in self.dataset_info.values()
            )
        ...
        if max_categorical_features is not None:
            ...
            self.max_categorical_features = max(
                info["categorical_features"] for info in self.dataset_info.values()
            )

        self.max_classes = max(
            info["num_classes"] for info in self.dataset_info.values()
        )

        print("Multi-dataset collection initialized:")
    ...
    def pad_features_to_unified_space(
        self, x_num: torch.Tensor, dataset_info: dict[str, Any]
        ...
        if self.max_numeric_features > C:
            padding = torch.full(
    ...
    def setup(self, stage: str | None = None):
        ...
        if stage == "fit" or stage is None:
            self.train_collection = MultiDatasetCollection(
                self.train_datasets, target_sequence_length=self.target_sequence_length
            )

            self.val_collection = MultiDatasetCollection(
                self.val_datasets, target_sequence_length=self.target_sequence_length
            )

    def train_dataloader(self):
        """Create training data loader with balanced sampling."""
        dataset_sizes = {
            name: len(dataset) for name, dataset in self.train_datasets.items()
        }
...
if __name__ == "__main__":
    ...
    for dataset_name, loader in val_loaders.items():
        ...
        print(
            f"{dataset_name} - x_num: {batch['x_num'].shape}, "
            f"dataset: {batch['dataset_name'][0]}"
        )
