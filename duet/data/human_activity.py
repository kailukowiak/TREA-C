...
class HumanActivityDataset(Dataset):
    ...
    def _download_and_prepare(self):
        ...
        total_samples = len(self.ACTIVITY_LABELS) * samples_per_activity
        ...
        for activity_id, activity in enumerate(self.ACTIVITY_LABELS):
            for _ in range(samples_per_activity):
                ...
                if "walking" in activity:
                    ...
                    acc_x = np.sin(freq * time_steps) + 0.3 * np.random.randn(self.seq_len)
                    acc_y = np.cos(freq * time_steps) + 0.3 * np.random.randn(self.seq_len) 
                    acc_z = 9.8 + 0.5 * np.sin(0.5 * freq * time_steps) + 0.2 * np.random.randn(self.seq_len)
                    
                    # Gyroscope shows turning movements
                    gyro_x = 0.2 * np.sin(1.5 * freq * time_steps) + 0.1 * np.random.randn(self.seq_len)
                    gyro_y = 0.2 * np.cos(1.5 * freq * time_steps) + 0.1 * np.random.randn(self.seq_len)
                    gyro_z = 0.1 * np.sin(0.8 * freq * time_steps) + 0.1 * np.random.randn(self.seq_len)
                    ...
                    acc_x = 9.8 + 0.1 * np.random.randn(self.seq_len)  # Gravity along x when laying
                    acc_y = 0.1 * np.random.randn(self.seq_len)
