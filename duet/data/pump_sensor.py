    ...
    def _create_synthetic_pump_data(self, num_samples):
        ...
        for sample_idx in range(num_samples):
            ...
            if fault_type == 0:  # Normal
                vib_x = 0.1 * np.random.randn(self.sequence_length) + 0.05 * np.sin(
                    2 * np.pi * 1 * t
                )
                vib_y = 0.1 * np.random.randn(self.sequence_length) + 0.05 * np.cos(
                    2 * np.pi * 1 * t
                )
                ...
                vib_x = 0.3 * np.random.randn(self.sequence_length) + 0.2 * np.sin(
                    2 * np.pi * 5 * t
                )
                vib_y = 0.3 * np.random.randn(self.sequence_length) + 0.2 * np.cos(
                    2 * np.pi * 5 * t
                )
                vib_z = 0.4 * np.random.randn(self.sequence_length)
                temp = 75 + 8 * np.random.randn() + 0.2 * t
                pressure = 90 + 10 * np.random.randn(
                ...
                rotation_freq = 2.5
                vib_x = 0.15 * np.random.randn(self.sequence_length) + 0.4 * np.sin(
                    2 * np.pi * rotation_freq * t
                )
                vib_y = 0.15 * np.random.randn(self.sequence_length) + 0.4 * np.cos(
                    2 * np.pi * rotation_freq * t
                )
                vib_z = 0.1 * np.random.randn(self.sequence_length) + 0.1 * np.sin(
                    2 * np.pi * rotation_freq * t
                )
                ...
                vib_x = 0.2 * np.random.randn(self.sequence_length) + 0.1 * np.sin(
                    2 * np.pi * 8 * t
                )
                vib_y = 0.2 * np.random.randn(self.sequence_length) + 0.1 * np.cos(
                    2 * np.pi * 8 * t
                )
                ...
                for _ in range(np.random.randint(5, 15)):
                    ...
                    vib_x[
                        max(0, impact_loc - 2) : min(
                            self.sequence_length, impact_loc + 2
                        )
            ...
            numeric_features = np.stack(
                [
                    ...
                    vib_y,
                    vib_z,
                    np.full(self.sequence_length, temp)
        ...
        self.cat_features = torch.zeros(
            num_samples, 3, self.sequence_length, dtype=torch.long
        )
