...
class AirQualityDataset(Dataset):
    ...
    def _download_and_prepare(self):
        ...
        total_samples = len(self.QUALITY_LABELS) * samples_per_class
        ...
        for quality_id, quality in enumerate(self.QUALITY_LABELS):
            for _ in range(samples_per_class):
                ...
                daily_pattern = 1 + 0.3 * np.sin(2 * np.pi * time_steps / 24 - np.pi/2)
                ...
                o3 = base_o3 * (1.5 - 0.5 * daily_pattern) + 10 * np.random.randn(self.seq_len)  # O3 anti-correlated
                co = base_co * daily_pattern + 0.5 * np.random.randn(self.seq_len)
                
                # Weather conditions (correlated with air quality)
                base_temp = 20 + 10 * np.sin(2 * np.pi * time_steps / 24)  # Daily temp cycle
                base_humidity = 60 - quality_id * 10  # Lower humidity = worse air quality
                base_pressure = 1013 + 5 * np.sin(2 * np.pi * time_steps / 24)
                
                temperature = base_temp + 3 * np.random.randn(self.seq_len)
                humidity = base_humidity + 0.3 * daily_pattern + 5 * np.random.randn(self.seq_len)
                pressure = base_pressure + 2 * np.random.randn(self.seq_len)
                wind_speed = (5 - quality_id * 1.5) + 2 * np.random.randn(self.seq_len)  # Higher wind = better air
                visibility = (15 - quality_id * 4) + 3 * np.random.randn(self.seq_len)  # Lower visibility = worse air
