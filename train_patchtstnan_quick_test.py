    ...
    def __init__(
        ...
        for well_name, group in self.data.groupby("well_name"):
            if len(group) >= self.seq_len + self.pred_horizon:
                ...
                valid_starts = range(
                    0, len(group) - self.seq_len - self.pred_horizon + 1, 10
                )
...
def main():
    ...
    data_module = W3DataModule(
        df, batch_size=batch_size, sequence_length=sequence_length
    )
    ...
    print(
        f"Data loading time for 3 batches: {data_time:.2f}s ({data_time / 3:.2f}s per batch)"
    )
