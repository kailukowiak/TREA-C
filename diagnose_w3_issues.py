...
def test_sequence_creation(df, numeric_cols):
    ...
    if samples:
        ...
        print(f"  x_num range: [{sample['x_num'].min():.3f}, {sample['x_num'].max():.3f}]")
        print(f"  x_num contains NaN: {torch.isnan(sample['x_num']).any()}")
        print(f"  x_num NaN percentage: {torch.isnan(sample['x_num']).float().mean()*100:.1f}%")
...
def test_model_forward_pass(samples, numeric_cols):
    ...
    batch_size = 4
    batch = {
        'x_num': torch.stack([samples[i % len(samples)]['x_num'] for i in range(batch_size)]),
        'x_cat': torch.stack([samples[i % len(samples)]['x_cat'] for i in range(batch_size)]),
    ...
    with torch.no_grad():
        ...
        try:
            ...
            if acc.item() == 1.0 and len(torch.unique(batch['y'])) > 1:
                print("⚠️  WARNING: Perfect accuracy on diverse labels - possible data leakage!")
