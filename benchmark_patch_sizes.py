...
def benchmark_patch_size(patch_size, C_num=20, T=64, batch_size=128, num_batches=10):
    ...
    print(f"Speedup vs patch_size=8: {get_speedup_ratio(patch_size, avg_time_per_batch):.2f}x")
...
def main():
    ...
    for patch_size in patch_sizes:
        if 64 % patch_size == 0:  # Only test valid patch sizes
            try:
                result = benchmark_patch_size(patch_size, C_num=20, T=64, batch_size=128)
                results.append(result)
            except Exception as e:
                print(f"Error with patch_size={patch_size}: {e}")
        else:
            print(f"Skipping patch_size={patch_size} (64 is not divisible by {patch_size})")
    ...
    print(f"{'Patch Size':<12} {'Patches':<8} {'Params':<10} {'Time/Batch':<12} {'Samples/s':<12} {'Speedup':<8}")
    print("-" * 70)
    ...
    for result in results:
        ...
        print(f"{result['patch_size']:<12} {result['n_patches']:<8} {result['params']:<10,} "
    ...
    if results:
        ...
        print(f"Expected speedup: {baseline_time / best_result['time_per_batch']:.2f}x faster" if baseline_time else "N/A")
        
        print(f"\nFor W3 dataset training:")
        print(f"- Use patch_size={best_result['patch_size']} for maximum speed")
        print(f"- This reduces transformer sequence from 8 to {best_result['n_patches']} patches")
        print(f"- Expected throughput: ~{best_result['samples_per_second']:.0f} samples/second")
