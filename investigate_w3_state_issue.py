...
def deep_state_analysis():
    ...
    try:
        ...
        combined_counts = relationship.group_by(["state", "class"]).len().sort("len", descending=True)
...
def explore_other_classification_options():
    ...
    try:
        ...
        feature_sample = df_lazy.select([
            "P-PDG", "P-TPT", "T-TPT", "QGL", "QBS"
        ]).head(100_000).collect()
        ...
        for col in ["P-PDG", "P-TPT", "T-TPT", "QGL", "QBS"]:
            if col in feature_sample.columns:
                ...
                if len(values) > 0:
                    ...
                    print(f"    Could create classes: Low<{q25:.2f}, Med={q25:.2f}-{q75:.2f}, High>{q75:.2f}")
    ...
    try:
        ...
        well_counts = well_sample["well_name"].value_counts().sort("count", descending=True)
...
def recommend_solutions():
    ...
    print("   This suggests it might be a status flag rather than a classification target")
